...
%         current_COTS_forControl(rfs,:)=recalc_COTS./META.COTS_control_sites(rfs,2);
    ...
    % YM (15/06/23): as it is, culling generates negative CoTS densities at the site level, which can create
    % negative densities at the reef level (in COTS_all_densities) and cause emergence of complex numbers in all
    % arrays derived from RESULT.COTS_all_densities (not just for the miscalculated reefs, also for those that
    % receive larvae from miscalculated reefs).
    % Easiest way to fix this is to force negative densities (per age class) to zero, although this does not solve
    % the miscalculations made within the culling algorithm.
    % Note that CoTS density after culling may be lower than the background_density, which is not consistent
    % with how natural die-off reset CoTS density (f_runmodel line 720).
    current_COTS(current_COTS<0)=0;
